...
class VehicleMPC():
    """
    VehicleMPC is the combination of two controllers
...
class MPCLateralController():
    """
    MPCLateralController implements lateral control using a MPC.
    """
    ...
    def run_step(self, waypoint):
        ...
        the vehicle towards a certain waypoin.
            ...
            +1 maximum steering to right
        """
        return self._mpc_control()

    def _mpc_control(self, velocity, waypoint, vehicle_transform):
        """
        Estimate the steering angle of the vehicle based on the MPC equations

            :param waypoint: target waypoint
            :param vehicle_transform: current transform of the vehicle
            :return: steering control in the range [-1, 1]
        """
        x = cp.Variable((self.state_dim, self.ph + 1))
        u = cp.Variable((self.control_dim, self.ph))

        # kinematic bycle model
        slip_angle = cp.Variable(self.ph)
        for h in range(self.ph):
            slip_angle[h] = cp.arctan(self.lr / (self.lf + self.lr) * cp.tan(u[h]))
        
        # state equation
        x0 = cp.vstack([vehicle_transform.location.x, vehicle_transform.location.y, vehicle_transform.rotation.yaw])
        for h in range(self.ph):
            x[:, h + 1] = x[:, h] + self.dt * cp.vstack([cp.cos(x[2, h] + slip_angle[h])*velocity, cp.sin(x[2, h] + slip_angle[h])*velocity, velocity*cp.cos(slip_angle[h])/(self.lr + self.lf)*cp.tan(u[h])])
        
        # get reference
        px_ref = waypoint.transform.location.x
        py_ref = waypoint.transform.location.y
        yaw_ref = waypoint.transform.rotation.yaw
        ...
        for h in range(self.ph):
            cost += cp.norm(cp.vstack([x[1, h] - py_ref, x[2, h] - yaw_ref]), 2) # for lateral control, only track y and yaw?
            constr += [cp.norm(u[h], 2) <= 0.1]
        constr += [x[:, 0] == x0] # final state?
